...
import { ChevronLeft, ChevronRight, Star, Play, BookOpen } from 'lucide-react';
import { NOVELS, TRENDING_NOVELS, NEW_RELEASES, COMPLETED_NOVELS, CONTINUE_READING, FEATURED_NOVELS } from '../data/novels';
import { GENRES } from '../data/genres';
import NovelCard from '../components/ui/NovelCard';
import SectionHeader from '../components/ui/SectionHeader';
import { useLanguage } from '../contexts/LanguageContext';

const BANNER_NOVELS = FEATURED_NOVELS.slice(0, 4);
...
export default function HomePage() {
  ...
  useEffect(() => {
    bannerRef.current = setInterval(() => setBannerIdx(i => (i + 1) % BANNER_NOVELS.length), 4000);
    return () => { if (bannerRef.current) clearInterval(bannerRef.current); };
  }, []);

  const prevBanner = () => setBannerIdx(i => (i - 1 + BANNER_NOVELS.length) % BANNER_NOVELS.length);
  const nextBanner = () => setBannerIdx(i => (i + 1) % BANNER_NOVELS.length);

  const featured = BANNER_NOVELS[bannerIdx];
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 space-y-10 animate-fade-in">
      ...
      <section>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 sm:gap-4">
          {TRENDING_NOVELS.slice(0, 6).map((novel, i) => (
      ...
      <section>
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 sm:gap-4">
          {NEW_RELEASES.slice(0, 6).map(novel => (
      ...
      <section className="pb-4">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-6 gap-3 sm:gap-4">
          {COMPLETED_NOVELS.slice(0, 6).map(novel => (
            <NovelCard key={novel.id} novel={novel} />
          ))}
          {NOVELS.slice(0, 4).map(novel => (
            <NovelCard key={`c-${novel.id}`} novel={novel} />
          ))}
        </div>
      </section>
